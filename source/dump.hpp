...
namespace Dump {
	...
	void Setup(std::shared_ptr<dbk::DumpingMenu> menu, float *progress_percent, bool *enable_buttons, dbk::DumpingMenu::DumpState *dumping_state) {
		...
		g_pathOut += strislandname.empty() ? "DA-" + util::getDreamAddrString(g_mainAddr) : ("DA-" + util::getDreamAddrString(g_mainAddr));
		mkdir(g_pathOut.c_str(), 0777);
		if (!strislandname.empty()) g_dumping_menu->LogAddLine("DA-" + util::getDreamAddrString(g_mainAddr));
